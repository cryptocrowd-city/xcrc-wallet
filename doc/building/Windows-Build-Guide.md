...
Below are some notes on how to build CRyptoCrowd Core for Windows.

The options known to work for building CRyptoCrowd Core on Windows are:

* On Linux using the [Mingw-w64](https://mingw-w64.org/doku.php) cross compiler tool chain. Ubuntu Bionic 18.04 is required
and is the platform used to build the CRyptoCrowd Core Windows release binaries.
...
Note that for WSL the CRyptoCrowd Core source path MUST be somewhere in the default mount file system, for
example /usr/src/cryptocrowd, AND not under /mnt/d/. If this is not the case the dependency autoconf scripts will fail.
...
way. This will install to `c:\workspace\cryptocrowd`, for example:

    make install DESTDIR=/mnt/c/workspace/cryptocrowd
